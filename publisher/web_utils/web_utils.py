...
class WebUtils():
    ...
    def scrape_issues(self, url):
        ...
        try:
            ...
            self._driver.close()
            print("ERROR: Failed to reach URL, check specified URL in constants.py")
            return False
        ...
        if "Buganizer" not in page_title or "componentid" not in page_title:
            if "MOMA Single Sign On" in page_title:
                ...
                while "Buganizer" not in page_title:
                    ...
                    page_title = soup.title.string
                    time.sleep(1)
                print("Close browser window and restart program now that you are logged in.")
                return False
        ...
        for tbody in soup.find_all('tbody'):
            for _tr in tbody.find_all('tr'):
                issue_link = "https://b.corp.google.com/issues/" + _tr.get('data-row-id')
    ...
    def visit_all_issues(self):
        ...
        if len(self._buganizer_issues) == 0:
            print("ERROR: Buganizer has not gathered issues and is attempting to visit them.")
            return
        for issue in self._buganizer_issues:
            ...
            while "@google.com" not in reporter:
                ...
                reporter = reporter_tag["aria-label"].replace("Reporter value is ", "")
